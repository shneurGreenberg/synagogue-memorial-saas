...
async function generateSavedViewThumbnail(snapshot, filename) {
  ...
  const panelFill = rgba(tile, tileAlpha);

  const svg = `
    <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
      <rect width="100%" height="100%" fill="#2a2118"/>
      <rect x="${panelX}" y="${panelY}" width="${panelW}" height="${panelH}" rx="10" fill="${escapeXml(panelFill)}" stroke="${escapeXml(primary)}" stroke-width="2"/>
      <rect x="${leftX}" y="${panelY + 6}" width="${leftW}" height="${panelH - 12}" rx="8" fill="${escapeXml(panelFill)}" stroke="${escapeXml(primary)}" stroke-width="1" opacity="0.98"/>
      <rect x="${centerX}" y="${panelY + 6}" width="${centerW}" height="${panelH - 12}" rx="8" fill="${escapeXml(panelFill)}" stroke="${escapeXml(primary)}" stroke-width="1" opacity="0.98"/>
      <rect x="${rightX}" y="${panelY + 6}" width="${rightW}" height="${panelH - 12}" rx="8" fill="${escapeXml(panelFill)}" stroke="${escapeXml(accent)}" stroke-width="1" opacity="0.98"/>
