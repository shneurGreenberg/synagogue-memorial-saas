...
function mixColor(hex, amount) {
  ...
  const channel = (value) => Math.round(value + (mix - value) * ratio);

  const r = channel(base.r);
  const g = channel(base.g);
  const b = channel(base.b);
...
async function generateSavedViewThumbnail(snapshot, filename) {
  ...
  const svg = `
    <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="${panelX}" y="${panelY}" width="${panelW}" height="${panelH}" rx="10" fill="${escapeXml(mixColor(tile, -0.35))}" stroke="${escapeXml(primary)}" stroke-width="2"/>
      <rect x="${leftX}" y="${panelY + 6}" width="${leftW}" height="${panelH - 12}" rx="8" fill="${escapeXml(mixColor(primary, -0.55))}" opacity="0.92"/>
      <rect x="${centerX}" y="${panelY + 6}" width="${centerW}" height="${panelH - 12}" rx="8" fill="${escapeXml(mixColor(tile, -0.2))}" opacity="0.95"/>
      <rect x="${rightX}" y="${panelY + 6}" width="${rightW}" height="${panelH - 12}" rx="8" fill="${escapeXml(mixColor(accent, -0.55))}" opacity="0.92"/>
      <rect x="${leftX + 8}" y="${panelY + 18}" width="${leftW - 16}" height="10" rx="3" fill="${escapeXml(accent)}" opacity="0.85"/>
      <rect x="${leftX + 8}" y="${panelY + 34}" width="${leftW - 16}" height="34" rx="4" fill="${escapeXml(text)}" opacity="0.18"/>
      <rect x="${leftX + 8}" y="${panelY + 74}" width="${leftW - 16}" height="16" rx="3" fill="${escapeXml(primary)}" opacity="0.55"/>
      ...
      ${tileRects}
      <circle cx="${rightX + rightW / 2}" cy="${panelY + 42}" r="10" fill="${escapeXml(accent)}" opacity="0.8"/>
      <rect x="${rightX + 8}" y="${panelY + 62}" width="${rightW - 16}" height="8" rx="2" fill="${escapeXml(text)}" opacity="0.35"/>
      <rect x="${rightX + 8}" y="${panelY + 76}" width="${rightW - 16}" height="8" rx="2" fill="${escapeXml(text)}" opacity="0.25"/>
      <rect x="${rightX + 8}" y="${panelY + 90}" width="${rightW - 16}" height="8" rx="2" fill="${escapeXml(text)}" opacity="0.25"/>
    </svg>
  `;
