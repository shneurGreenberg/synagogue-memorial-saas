...
async function loadSynagogueBoard(slug) {
  ...
  // Ensure slideshow images have identifiers for admin operations
  if (synagogue.slideshow && Array.isArray(synagogue.slideshow.images)) {
    synagogue.slideshow.images = synagogue.slideshow.images.map((slide, index) => {
      if (slide && !slide._id && !slide.id) {
        // Use index as fallback identifier for legacy slides without _id
