...
function buildMissedYahrzeitMessage({
  synagogueName,
  deceasedName,
  contact,
  donationUrl,
  ...
  if (safeLang === 'he') {
    let message = `${salutation},\n\n`;
    message += `בשבוע האחרון, ביום השנה של ${deceasedName}, זכרנו אותו/ה בתפילה${community ? ` ב${community}` : ''}. `;
    message += 'נזכרנו את נשמתו/ה בתפילה ובקדיש, ומבקשים כי זכרו/ה יהיה לברכה.\n\n';
    if (donationUrl) {
      message += `נהוג ביום השנה לתת צדקה לעילוי נשמת הנפטר. ניתן לתרום כאן: ${donationUrl}`;
    } else {
  ...
  if (safeLang === 'en') {
    let message = `${salutation},\n\n`;
    message += `Earlier this week, on the yahrzeit of ${deceasedName}, we remembered them in prayer${community ? ` at ${community}` : ''}. `;
    message += 'We prayed for the elevation of their soul and that their memory be for a blessing.\n\n';
    if (donationUrl) {
      message += `It is customary to give charity on a yahrzeit. You may contribute here: ${donationUrl}`;
    } else {
  ...
  let message = `${salutation},\n\n`;
  message += `На этой неделе, в день годовщины ${deceasedName}, мы помянули его/её в молитве${community ? ` в ${community}` : ''}. `;
  message += 'Мы молились об упокоении души и о том, чтобы память о нем/ней была благословением.\n\n';
  if (donationUrl) {
    message += `В день годовщины принято давать цдаку. Пожертвование можно сделать по ссылке: ${donationUrl}`;
  } else {
...
function buildAdminReminderBody({
  ...
  entries.forEach((entry, index) => {
    ...
    if (entry.yahrzeitIsHebrewToday && !entry.yahrzeitIsGregorianToday) {
      lines.push(safeLang === 'he' ? '   יום שנה עברי' : safeLang === 'en' ? '   Hebrew yahrzeit' : '   Еврейская годовщина');
    } else if (entry.yahrzeitIsGregorianToday && !entry.yahrzeitIsHebrewToday) {
      lines.push(safeLang === 'he' ? '   יום שנה לועזי' : safeLang === 'en' ? '   Civil date yahrzeit' : '   Гражданская годовщина');
    } else if (entry.yahrzeitIsHebrewToday && entry.yahrzeitIsGregorianToday) {
      lines.push(safeLang === 'he' ? '   יום שנה עברי ולועזי' : safeLang === 'en' ? '   Hebrew and civil yahrzeit' : '   Еврейская и гражданская годовщина');
    }
