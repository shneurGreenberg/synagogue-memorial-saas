...
const { normalizeFontScales, normalizeTileOpacity, normalizeLogoBorderRadius } = require('./theme-typography');
...
function enrichSynagogueForAdmin(synagogue) {
  ...
  doc.theme = {
    ...
    logoBorderRadius: normalizeLogoBorderRadius(doc.theme && doc.theme.logoBorderRadius),
  ...
  doc.slideshow = {
    enabled: !!(doc.slideshow && doc.slideshow.enabled),
    interval: (doc.slideshow && doc.slideshow.interval) || 10,
    mainDuration: (doc.slideshow && doc.slideshow.mainDuration) || 30,
    images: (doc.slideshow && Array.isArray(doc.slideshow.images)) 
      ? doc.slideshow.images.map((slide, index) => {
          // Ensure legacy slides without _id get a fallback ID for deletion
