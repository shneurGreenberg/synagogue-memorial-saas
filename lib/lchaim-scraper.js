...
function fetchText(url) {
  return new Promise((resolve, reject) => {
    ...
    const request = client.get(url, {
      ...
      insecureHTTPParser: !url.startsWith('https'),
    }, (response) => {
...
function extractAphorism(html) {
  ...
  return stripHtml(withoutTable)
    .replace(/\[\s*\d+\s*\]/g, '')
    .replace(/Notes:\s*\(\s*Back to text\s*\)/gi, '')
    .replace(/\s+/g, ' ')
    .trim();
}
...
async function fetchLchaimPage(date) {
  ...
  const path = `/cgi-bin/lessons.cgi?Center=y&d2=1&date=${dateParam}&width=35`;
  const hosts = [
    `http://www.lchaimweekly.org${path}`,
    `http://lchaimweekly.org${path}`,
  ];
  ...
  throw lastError || new Error('Failed to fetch lchaim lessons');
}

async function fetchRambamTitle(date, chapters) {
  const dateParam = formatLchaimDate(date);
  const param = chapters === 3 ? 'r3=3' : 'r1=1';
  const url = `http://www.lchaimweekly.org/cgi-bin/lessons.cgi?date=${dateParam}&${param}&width=35&Center=y`;
