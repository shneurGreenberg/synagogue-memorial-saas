 ...
 * - OPENROUTER_API_KEY  OpenRouter key. This is the supported vision path.
 * - SCAN_OCR_MODEL      OpenRouter model id. Default is google/gemini-3.8-flash,
 *                       which read a curved cemetery stone accurately in about 7s.
 *                       Optional override: deepseek/deepseek-v4.1-flash (slower, and
 *                       it invented years and patronymics on that stone).
...
const OPTIONAL_DEEPSEEK_OPENROUTER_MODEL = 'deepseek/deepseek-v4.1-flash';
...
const GRAVE_OCR_PROMPT = [
  ...
  'Letters are often carved in a curve. Read along the arc. A portrait photo often covers part of the stone; read text that is partly hidden beside or under the portrait.',
  'Do not invent names, dates, years, or patronymics. If a year or a father\'s name is unclear or absent, use null or an empty string. Never fill a plausible year that is not carved.',
...
function scanModelId(env, fallback) {
  return String(env.SCAN_OCR_MODEL || env.GRAVE_OCR_MODEL || fallback).trim();
}
...
function resolveGraveOcrConfig(env = process.env) {
  ...
  if (provider === 'deepseek') {
    return {
      ...
      model: scanModelId(env, env.DEEPSEEK_MODEL || DEFAULT_DEEPSEEK_MODEL),
      url: DEEPSEEK_URL,
    };
  }

  if (provider === 'gemini') {
    const model = scanModelId(env, env.GEMINI_MODEL || DEFAULT_GEMINI_MODEL);
    return {
      provider: 'gemini',
  ...
  return {
    ...
    model: scanModelId(env, env.OPENROUTER_MODEL || DEFAULT_OPENROUTER_MODEL),
    url: OPENROUTER_URL,
  };
}
...
module.exports = {
  ...
  OPTIONAL_DEEPSEEK_OPENROUTER_MODEL,
