...
export function normalizeCandlePalette(value) {
  const key = String(value || 'classic').trim().toLowerCase();
  return Object.prototype.hasOwnProperty.call(CANDLE_PALETTES, key) ? key : 'classic';
}

export function resolveCandlePalette(value) {
  return CANDLE_PALETTES[normalizeCandlePalette(value)];
}

export function buildCandlePaletteCssVars(palette) {
